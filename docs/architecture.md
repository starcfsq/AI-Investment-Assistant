...
| 故障 | 处理 |
|---|---|
| **数据层：akshare 网络/接口失败** | 数据获取采用缓存优先策略：命中未过期缓存直接返回（status=cached）；缓存未命中则抓取 akshare，失败重试 1 次（status=ok_retry）；两次均失败则返回空数据并标记 status=missing，该项跳过。上层据此降级（详见 §2 数据流时序）。 |
...
防前视偏差：决策时刻只用当时可得数据；训练/验证时间窗分割；权重更新设收敛阈值（验证窗胜率提升不超过 `1e-9` 则不更新）。设计文档 §6.10 提出以因子 IC 监控因子有效性，作为后续评估增强项，当前实现聚焦于胜率与超额收益指标。
