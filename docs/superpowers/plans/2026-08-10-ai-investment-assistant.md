...
def backtest_sectors(sector_hist: dict[str, pd.DataFrame], bench: pd.DataFrame,
    ...
    t = start + pd.Timedelta(days=lookahead_days)
    wins, samples = 0, 0
    excess_returns = []
    while t + pd.Timedelta(days=lookahead_days) <= end:
        rows = []
        ...
        if len(rows) < 3:
            t += pd.Timedelta(days=lookahead_days)
            continue
        rs = _minmax([r["rs_raw"] for r in rows])
        mo = _minmax([r["momentum_raw"] for r in rows])
        for i, r in enumerate(rows):
        ...
        for r in top3:
            ...
            h["date"] = pd.to_datetime(h["date"])
            start_p = h[h["date"] <= t]
            end_p = h[(h["date"] > t) & (h["date"] <= t + pd.Timedelta(days=lookahead_days))]
        ...
        b_end = bench[(bench_dates > t) & (bench_dates <= t + pd.Timedelta(days=lookahead_days))]
